...
using capygram.Auth.MessageBus.Event;
...
namespace capygram.Auth.DependencyInjection.Extensions
{
    public static class ServicesCollectionExtensions
    {
        ...
        {
            services.AddMediatR(cgf => cgf.RegisterServicesFromAssembly(typeof(ServicesCollectionExtensions).Assembly));
            return services;
        ...
        public static IServiceCollection ConfigurationMasstransit(this IServiceCollection services, IConfiguration config)
        {
            ...
            services.AddMassTransit(cfg =>
            {
                ...
                cfg.UsingRabbitMq((context, bus) =>
                {
                    ...
                    //// Receive for user
                    bus.ReceiveEndpoint(massOp.UserQueueName, re =>
                    {
                        re.ConfigureConsumeTopology = false;
                        re.ConfigureConsumer<UserMediaNotificationConsumer>(context);

                        re.Bind(massOp.ExchangeMediaName, e =>
                        {
                            e.Durable = true; // default true
                            e.AutoDelete = false; // default: false
                            e.RoutingKey = NotificationMediaType.user;
