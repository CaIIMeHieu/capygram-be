...
namespace capygram.Auth.Services
{
    public class UserServices : IUserServices
    {
        ...
        public async Task<Result<User?>> GetUserByName(string UserName)
        {
            ...
            return Result<User?>.CreateResult(true, new ResultDetail("200", "Success"), result);
        }
        ...
        public async Task<Result<User>> GetUserByUserID(Guid UserID)
        {
        ...
        public async Task<Result<string>> UpdateProfile(UserUpdateProfileDto request)
        {
            ...
            userNotification.User.AvatarUrl = request.AvatarUrl != null ? request.AvatarUrl : user.Profile.AvatarUrl;
            using var source = new CancellationTokenSource(TimeSpan.FromSeconds(30));
            await _publishEndpoint.Publish(userNotification, source.Token);
            return "Update User Successful";
        }

        public async Task<Result<string>> UploadAvatar( List<IFormFile> fileToUpload , Guid userId )
        {
            ...
            var MediaUploadNotification = new ImageUploadNotification
            {
                Id = userId,
                Data = fileDataList, 
                Description = "send message to media service",
                Name = "upload image",
                TimeStamp = DateTimeOffset.UtcNow, 
                Type = NotificationMediaType.user,
                CloudinaryFolder = CloudinaryFolder.Capygram_User,
            };

            using var source = new CancellationTokenSource(TimeSpan.FromSeconds(30));
            await _publishEndpoint.Publish(MediaUploadNotification, source.Token);
